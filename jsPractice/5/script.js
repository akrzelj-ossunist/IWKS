const json = [ 
 ...
 /*Napisati funkciju koja vraća niz stringova u kojima su samo imena država (name.common). Koristiti .map funkciju i arrow funkcije.*/ 
 const zadatak1 = document.getElementsByName('zdk1')[0]

 zadatak1.addEventListener('click', () =>{
    const names = json.map(element => element.name.common);
    console.log(names)
 })

 /*Napisati funkciju koja vraća sve države iz europe. Koristiti .filter funkciju i arrow funkcije.*/
 const zadatak2 = document.getElementsByName('zdk2')[0]

 zadatak2.addEventListener('click', () => {
    let countryEurope = json.filter(element => element.region === 'Europe')
    countryEurope = countryEurope.map(element => element.name.common)
    console.log(countryEurope)
})

 /*Napisati funkciju koja vraća ukupnu površinu svih država koje su u Europi. Koristiti .reduce funkciju i arrow funkcije.*/
 const zadatak3 = document.getElementsByName('zdk3')[0]

 zadatak3.addEventListener('click', () => {
    let countryEurope = json.filter(element => element.region === 'Europe')
    countryEurope = countryEurope.map(element => element.area)
    countryEurope = countryEurope.reduce((a,b) => a+b)
    console.log(countryEurope)
})

 /*Napisati funkciju koja vraća državu čiji 
 se glavni grad zove Pyongyang. Ukoliko takva država ne postoji vratiti null. 
 Koristiti .find funkciju i arrow funkcije.*/
 const zadatak4 = document.getElementsByName('zdk4')[0]

 zadatak4.addEventListener('click', () => {
    const Capital = 'Pyongyang'
    const findCountry = json.find(element => element.capital == Capital) ? json.find(element => element.capital == Capital).name.common : null;
